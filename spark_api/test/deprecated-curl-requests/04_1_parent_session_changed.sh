...
curl -X POST http://localhost:4444/addSql \
    -H 'Content-Type: application/json' \
    -d '{"session_id": "0000", "node_id": "n0001", "prev": "0000-0000-0000", "query": "select * from {df} where tld = '\''com'\'' OR tld = '\''org'\''", "query_name": "filter", "query_params_json": ["df"]}' \
    -w '\nTotal: %{time_total}s\n\n'
...
curl -X POST http://localhost:4444/addSql \
    -H 'Content-Type: application/json' \
    -d '{"session_id": "0000", "node_id": "n0002", "prev": "n0001", "query": "select * from {df} where registrar = '\''GoDaddy.com, LLC'\'' OR registrar = '\''NameCheap, Inc.'\'' OR registrar = '\''unknown'\''", "query_name": "filter", "query_params_json": ["df"]}' \
    -w '\nTotal: %{time_total}s\n\n'
...
curl -X POST http://localhost:4444/addSql \
    -H 'Content-Type: application/json' \
    -d '{"session_id": "0001", "node_id": "n0001", "prev": "0000-0000-0000", "query": "select * from {df} where registrar = '\''NameCheap, Inc.'\''", "query_name": "filter", "query_params_json": ["df"]}' \
    -w '\nTotal: %{time_total}s\n\n'
...
curl -X POST http://localhost:4444/addSql \
    -H 'Content-Type: application/json' \
    -d '{"session_id": "0000", "node_id": "n0003", "prev": "n0002", "query": "select * from {df} where tld = '\''org'\''", "query_name": "filter", "query_params_json": ["df"]}' \
    -w '\nTotal: %{time_total}s\n\n'

echo "get rebuild status for 0001"
curl -X GET http://localhost:4444/getSessionStatus/0001 \
    -w '\nTotal: %{time_total}s\n\n'
